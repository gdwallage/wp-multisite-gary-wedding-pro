 ...
 * Version: 1.81.0
 * Fixes: About Me 3D Frame Sync and Footer Menu Hard Reset.
 */
...
function gary_get_bookly_service_data( $service_id ) {
    ...
    $service = $wpdb->get_row( $wpdb->prepare( "SELECT price, duration FROM $table_name WHERE id = %d", $service_id ) );
    if ( $service ) {
        $hours = floor($service->duration / 3600);
        $mins  = ($service->duration % 3600) / 60;
        $duration_label = ($hours > 0 ? $hours . 'h ' : '') . ($mins > 0 ? $mins . 'm' : '');
        return array( 'price' => (float) $service->price, 'duration' => $duration_label );
    }
    return false;
}
...
add_action( 'wp_head', function() {
    ...
    <style type="text/css">
        ...
        .entry-title, .about-title, .archive-header h1 { margin-top: 100px !important; margin-bottom: 60px !important; }
...
function gary_send_performance_headers() {
    ...
    header( "Link: <{$template_uri}/style.css?ver=1.81.0>; rel=preload; as=style", false );
}
add_action( 'send_headers', 'gary_send_performance_headers' );

function gary_wedding_scripts() { wp_enqueue_style( 'gary-wedding-style', get_stylesheet_uri(), array(), '1.81.0' ); }
