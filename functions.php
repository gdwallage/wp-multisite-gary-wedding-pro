...
{
    if (isset($_POST['gary_bookly_id']))
        update_post_meta($post_id, '_gary_bookly_id', $_POST['gary_bookly_id']);
    if (isset($_POST['gary_service_subtitle']))
        update_post_meta($post_id, '_gary_service_subtitle', $_POST['gary_service_subtitle']);
    if (isset($_POST['gary_service_highlights']))
        update_post_meta($post_id, '_gary_service_highlights', $_POST['gary_service_highlights']);

    if (isset($_POST['gary_retail_value_override']))
        update_post_meta($post_id, '_gary_retail_value_override', $_POST['gary_retail_value_override']);
    if (isset($_POST['gary_booking_shortcode']))
        update_post_meta($post_id, '_gary_booking_shortcode', $_POST['gary_booking_shortcode']);

    for ($i = 1; $i <= 8; $i++) {
        if (isset($_POST["gary_sub_service_$i"]))
            delete_post_meta($post_id, "_gary_sub_service_$i");
        if (isset($_POST["gary_paid_service_$i"]))
            delete_post_meta($post_id, "_gary_paid_service_$i");
    }
}
...
function gary_get_manual_savings_fallback($title)
{
    ...
    if (strpos($title, 'story') !== false)
        return 100;
    if (strpos($title, 'ceremony') !== false)
        return 50;
    if (strpos($title, 'celebration') !== false)
        return 100;
    if (strpos($title, 'registry') !== false)
        return 40;
        ...
        if (is_array($native_sub)) {
            foreach ($native_sub as $rel) {
                ...
                if ($sub_data && !in_array($rel->sub_service_id, $processed_ids)) {
                    ...
                    $inclusions[] = array(
                        'page_id' => $p_id,
                        'bookly_id' => $rel->sub_service_id,
                        'title' => $sub_data['title'],
                        'clean_title' => gary_clean_service_name($sub_data['title']),
                        'price' => $unit_p,
                        'duration' => $unit_d,
                        'info' => $sub_data['info'],
                        'thumbnail' => $thumb,
    ...
    if ($bookly_id && $wpdb->get_var("SHOW TABLES LIKE '$table_inc'") == $table_inc) {
        ...
        if (is_array($db_inclusions)) {
            foreach ($db_inclusions as $db_inc) {
                if (in_array($db_inc->included_id, $processed_ids))
                ...
                if ($sub_data) {
                    ...
                    $inclusions[] = array(
                        'page_id' => $p_id,
                        'bookly_id' => $db_inc->included_id,
                        'title' => $sub_data['title'],
                        'clean_title' => gary_clean_service_name($sub_data['title']),
                        'price' => (float) $sub_data['price'],
                        'duration' => (int) $sub_data['duration'],
                        'info' => $sub_data['info'],
                        'thumbnail' => $thumb,
                        'permalink' => $p_id ? get_permalink($p_id) : '#'
                    );
    ...
    if ($credits) {
        ...
        foreach ($credits as $c) {
            ...
            if (!$s_data)
                continue;
            ?>
            <div class="gw-credit-card"
                style="border: 2px solid var(--brand-gold-light); padding: 20px; background: #fff; position:relative;">
                ...
                <a href="/booking/?redeem=<?php echo $c->id; ?>" class="btn-black"
                    style="display:block; background:#000; color:#fff; text-decoration:none; text-align:center; padding:10px; font-size:0.7rem; text-transform:uppercase; letter-spacing:1px; font-weight:700;">Book
                    Using Credit</a>
            </div>
            <?php
        }
...
function gary_consume_credits_on_order_complete($order_id)
{
    ...
    if ((float) $order->get_total() > 0)
        return; // Only process zero-price redemptions
